...
def create_transaction():
    ...
    app.logger.error({'gw': app.config['gw']})
    app.logger.error({'url': urllib.parse.urljoin(app.config['gw'], 'transactions')})
    response = requests.post(urllib.parse.urljoin(app.config['gw'], 'transactions'), json=json_data, timeout=3)
...
def calculate_amount():
    ...
    response = requests.get(urllib.parse.urljoin(app.config['gw'], 'amount'), { 'blockchain_address': my_blockchain_address }, timeout=3)
