...
class BlockChain(object):
    def __init__(self, blockchain_address=None, port=None):
        self.transaction_pool = [] # 未確定トランザクションの一覧
        self.chain = [] # ブロック一覧
        self.neighbours = []
        self.create_block(0, self.hash({})) # ジェネシスブロックの作成
        self.blockchain_address = blockchain_address # マイナーの報酬受取用アドレスを保持
        self.port = port
    ...
    def add_transaction(self, sender_blockchain_address,
        ...
        if sender_blockchain_address == MINING_SENDER:
            ...
            return True
        elif self.verify_transaction_signature(sender_public_key, signature, transaction):
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if block['previous_hash'] != self.hash(pre_block):
                return False
            if not self.valid_proof(block['transactions'], block['previous_hash'], block['nonce'], MINING_DIFFICULTY):
                return False
            pre_block = block
            current_index += 1
        return True
